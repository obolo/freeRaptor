 ...
 */
void LTEnc(uint16_t X, uint16_t K, uint16_t* C, uint16_t triple[3], uint16_t G);

/**
 * Function that buils the LDPC matrix
 * 
 */
int build_LDPC_mat(int K, int S, uint8_t C[K]);

/**
 * Function that builds the Half matrix
 */
int build_Half_mat(int K, int H, int S, uint8_t* C);

/**
 * Function that builds the Constraints matrix
 */
int build_constraints_mat(uint16_t K, uint16_t S, uint16_t L, R10* enc);
